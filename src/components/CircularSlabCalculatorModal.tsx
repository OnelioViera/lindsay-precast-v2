...
interface CircularSlabCalculatorModalProps {
  isOpen: boolean;
  onClose: () => void;
}
...
export default function CircularSlabCalculatorModal({
  isOpen,
  onClose,
}: CircularSlabCalculatorModalProps) {
  const [dimensions, setDimensions] = useState<Dimensions>({
    outerDiameter: { value: "0", unit: "feet" },
    innerDiameter: { value: "0", unit: "feet" },
  ...
  const calculateConcrete = () => {
    const outerD = parseFloat(dimensions.outerDiameter.value);
    const innerD = parseFloat(dimensions.innerDiameter.value);
    const h = parseFloat(dimensions.depth.value);
    const q = parseFloat(dimensions.quantity);

    if (isNaN(outerD) || isNaN(innerD) || isNaN(h) || isNaN(q)) {
      return;
    }
